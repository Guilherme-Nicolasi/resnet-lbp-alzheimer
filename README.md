...
Para o treinamento da Rede Neural Convolucional (CNN, da tradução de Convolutional Neural Network), selecionamos 5 (cinco) conjuntos com 90% de imagens randomizadas, então usadas no modelo, passando por 5 épocas, e, posteriormente, obtivemos as features de mínimos locais, com a descida de gradiente estocástico.
...
A diferença que a doença traz ao cérebro, visualmente, é significativa, principalmente em condições mais avançadas. No entanto, para que seja possível notar tal diferença, é necessário ter uma amostra do cérebro do paciente. Para isso, são feitas ressonâncias magnéticas, ferramenta que nos ajuda a projetar o objeto de estudo. O revés, da utilização dessa tecnologia, surge a partir das limitações visuais e analíticas do uso dessa tecnologia, já que, em MRI (Magnetic Ressonance Images), o que era perceptivo passa a ser difícil de identificar. Logo, são necessários métodos que possibilitam analaisar tais imagens com maior precisão e detalhamento para se obter diagnósticos mais confiáveis. Portanto, para que isso seja possível há vários caminhos a serem tomados, por exemplo:</p>
